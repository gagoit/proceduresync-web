(function() {
  ...
  $.fn.S3Uploader = function(options) {
    ...
    settings = {
      ...
      callback_set_fileinfo: null /*Custom callback for setting file-information when start upload file (DatPB) */
    };
    ...
    setUploadForm = function() {
      return $uploadForm.fileupload({
        ...
        start: function(e) {
          /** 
          * Try to set file information (name) when start uploading.
          * *author: DatPB
          **/
          try{
            if(settings.callback_set_fileinfo){
              settings.callback_set_fileinfo(current_files[0].files[0].name, current_files[0].files[0].size);
            }
          }catch(e){
